...
const createIcon = (iconUrl = "https://cdn-icons-png.flaticon.com/512/684/684908.png") =>
    new L.Icon({
        iconUrl,
...
export default function MapSearchCompare() {
    ...
    const searchPlaces = async (searchText: string, cat: string) => {
        try {
            ...
            const res = await axios.get<NominatimPlace[]>("https://nominatim.openstreetmap.org/search", {
                params: {
                    q,
                    format: "json",
                    addressdetails: 0,
                    ...
                    <div className="flex flex-wrap gap-3 mt-2 items-center justify-evenly">
                        ...
                        {["restaurant", "hotel", "museum"].map((cat) => (
                            <button
                                ...
                                className={`w-24 text-[13px] lg:text-[16px] lg:w-60 px-4 py-3 rounded-[10px] hover:opacity-[.8] ${category === cat ? "bg-primary text-white" : "bg-white text-secondary"
                                    }`}
                            >
                                {cat.charAt(0).toUpperCase() + cat.slice(1)}
                            </button>
                        ))}
                        <div className="text-[16px] w-28 lg:w-60 text-center md:text-xl text-primary font-bold bg-secondary-foreground p-2 rounded-[8px]">Results: {places.length}</div>
                    </div>
                </div>
            ...
            <div className="flex h-screen w-screen text-input">
                ...
                <main className="flex-1 relative">
                    <MapContainer center={[center.lat, center.lon]} zoom={13} style={{ height: '100%', width: '100%' }}>
                        ...
                        {/* Render markers */}
                        {places.map((p) => (
                            <Marker key={p.place_id} position={[+p.lat, +p.lon]} icon={pinIcon}>
                                <Popup>
                                    <div className="w-64">
                                        <div className="font-semibold text-sm">{p.display_name.split(",")[0]}</div>
                                        <div className="text-xs text-gray-600 mb-2">{p.display_name}</div>
                                        <div className="flex gap-2 mt-2">
                                            <button
                                                className="text-xs bg-blue-600 text-white px-2 py-1 rounded"
                                                onClick={() => {
                                                    // simulate navigation to booking page
                                                    alert(`Navigate to booking for: ${p.display_name.split(',')[0]}`);
                                                }}
                                            >
                                                Book
                                            </button>
